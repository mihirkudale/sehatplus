...
const Services = () => {
  ...
  return (
    <section id="services" className="bg-[#FAF9F6] py-24">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8 text-center md:text-left">
          <div className="max-w-2xl">
            <h2 className="text-4xl md:text-5xl font-serif mb-6 leading-tight text-[#1A1A1A]">
              ...
              <span className="text-[#2D5A27] italic font-normal">Choose Your Path.</span>
            </h2>
          ...
          {services.map((service, index) => (
            ...
            >
              <div className="p-10 flex flex-col h-full bg-white rounded-[2.4rem] overflow-hidden">
                <div className="inline-block px-4 py-1 rounded-full bg-[#FAF9F6] text-[#2D5A27] text-xs font-bold tracking-widest uppercase mb-8 w-fit mx-auto md:mx-0">
                  {service.tag}
                </div>
                
                <div className="mb-8 p-4 bg-[#2D5A27]/5 rounded-2xl w-fit group-hover:bg-[#2D5A27] transition-colors duration-500 mx-auto md:mx-0">
                  <div className="text-[#2D5A27] group-hover:text-white transition-colors duration-500">
                    {service.icon}
                  </div>
                </div>

                <h3 className="text-2xl font-bold mb-4 group-hover:text-[#2D5A27] transition-colors duration-300 text-center md:text-left">
                  {service.title}
                </h3>
                ...
                <a 
                  href="#booking" 
                  className="flex items-center gap-2 text-sm font-bold text-[#2D5A27] uppercase tracking-wider group/link justify-center md:justify-start"
                >
                  Explore More
                  <div className="w-6 h-[2px] bg-[#2D5A27] group-hover/link:w-10 transition-all duration-300" />
                </a>
              </div>
            </div>
          ))}
